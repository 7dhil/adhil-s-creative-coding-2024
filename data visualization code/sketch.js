...
function preload() {
  phoneData = loadTable('phone_sales.csv', 'csv', 'header');
}
...
function setup() {
  ...
  let startY = 50;
  let barWidth = 30;
  maxSales = getMaxSales();

  // Draw bars for each phone brand
  for (let i = 0; i < phoneData.getRowCount(); i++) {
    let brand = phoneData.getString(i, 'Brand');
    let sales = parseFloat(phoneData.getString(i, 'Sales'));

    let x = startX + i * 100;
    let barHeight = map(sales, 0, maxSales, 0, 300);
